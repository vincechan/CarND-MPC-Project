...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += 10000 * CppAD::pow(vars[cte_start + t] - ref_cte, 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // result the predicted x and y values, x will be stored in even
  // elements and y will be stored in odd elements
  for (int i = 0; i < N - 1; i++) {
